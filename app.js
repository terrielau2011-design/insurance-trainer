...
function renderTagPanel() {
  ...
  if (!container) return;
  container.innerHTML = productData.tags.map(function(t) {
    return '<label class="tag-label">' +
      '<input type="checkbox" value="' + t.tag_name + '" onchange="onTagChange()">' +
  ...
  container.innerHTML = list.map(function(p) {
    ...
    return '<div class="product-card" data-prod-id="' + p.prod_id + '">' +
      ...
      '<div class="card-actions">' +
        '<button class="btn btn-ghost" onclick="showIncomeChart(\'' + p.prod_id + '\')">收益曲線</button>' +
        '<button class="btn btn-ghost" onclick="showIRRChart(\'' + p.prod_id + '\')">IRR對比</button>' +
        '<button class="btn btn-primary" onclick="addToReport(\'' + p.prod_id + '\')">加入報告</button>' +
      '</div>' +
    '</div>';
  }).join('');
...
function onCompareToggle(prodId) {
  ...
  if (idx === -1) {
    if (comparedProductIds.length >= 4) {
      alert('最多同時對比 4 款產品');
      const cb = document.querySelector('input[value="' + prodId + '"]');
...
function renderComparePanel() {
  ...
  rows.forEach(function(r) {
    html += '<tr><td><strong>' + labels[r] + '</strong></td>';
...
function showIRRChart(prodId) {
  ...
  const cfg = chartConfig.irr_compare_chart;
  const irr = parseFloat(product.irr_20) || 0;
  const avgIRR = 4.5;
  const option = {
    backgroundColor: chartConfig.theme.backgroundColor,
